...
import { createSchema, ProductModel, OrderModel, createLoaders } from "./schema"
...
describe("schema", () => {
  ...
  describe("execution", () => {
    ...
    const subject = async () =>
      await execute({
        schema: createSchema(),
        contextValue: { productModel, orderModel, loaders: createLoaders({ productModel, orderModel }) },
        document,
        variableValues,
      })
    ...
    describe("when product to look up exists", () => {
      ...
      test("object in list", async () => {
        ...
        expect(productFindBy).not.toBeCalledTimes(5)
